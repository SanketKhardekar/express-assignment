        ...
        if (result.length > 0) {
          ...
          db.query(query, user, (err, result) => {
            if (!err) {
              res
                .status(200)
                .json({ message: "Registration Successfull", userId: newId });
              return;
            } else {
              console.log(err);
              res
      ...
      if (!err) {
        if (result.length > 0) {
          if (await bcrypt.compare(req.body.password, result[0].password)) {
            res
              .status(200)
              .json({ message: "Login Sucessfull", userId: result[0].id });
            return;
          } else {
          ...
          if (result.length <= 0) {
            ...
            db.query(query, (err, result) => {
              if (!err) {
                ...
                res
                  .status(409)
                  .json({ message: "Deletion Failed Try Again Later" });
                return;
              }
            });
          }
          ...
          if (result.length <= 0) {
            ...
            db.query(query, [updateData, req.query.id], (err, result) => {
              if (!err) {
                ...
                res
                  .status(409)
                  .json({ message: "Update Failed Try Again Later" });
                return;
              }
            });
          }
